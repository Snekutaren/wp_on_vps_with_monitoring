...
    fi
    ...
    # Initialize variables with default values before processing options
    local CUSTOM_APP_BASE_NAME_ARG=""
    local APP_NAME_SUFFIX_ARG="" # Initialize to empty string for no suffix by default
    local PORT_OFFSET_ARG=0      # Initialize to 0 so no offset is applied by default
...
          # If -n is provided, append it directly (e.g., -n 0 -> _0, -n 1 -> _1)
          APP_NAME_SUFFIX_ARG="_$OPTARG"
          ;;
        o) # Global port offset (added to all default ports)
          # Set PORT_OFFSET_ARG directly to the provided value
...
    # Apply global port offset to all relevant ports
    # These calculations now correctly use PORT_OFFSET_ARG which is either 0 or the provided value
